...
1. be present in the `trombone_keys` database table with a unique identifier and its secret key; as well as
...
Message authentication can be disabled using the `-x` command line switch. This is not recommended in a production environment, since it renders the system vunerable to unauthorized access.

To bypass HMAC authentication specifically for requests originating from localhost, instead use the `-t`, or `--trust-localhost` option. 
