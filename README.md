...
This type of variable must also be present in the route's uri pattern, where it is bound to a specific path segment. The variable name can only contain alphanumeric characters, hyphens and underscores. Furthermore, it is always prefixed with a single colon to make the distinction clear from ordinary request body placeholders, which are explained next.    
