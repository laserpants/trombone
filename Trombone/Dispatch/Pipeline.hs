...
dispatchPipeline :: Pipeline -> [(Text, EscapedText)] -> Value -> Dispatch IO RouteResponse
dispatchPipeline q ps (Array a) =
    liftM resp $ mapM (dispatchPipeline q ps) (Vect.toList a)
  where 
    val (RouteResponse _ _ x) = x
...
dispatchPipeline (Pipeline pcs conns _) ps obj = do
  ...
  where 
    ...
    initMq = return $ broadcast (outgoingConns In conns) start
...
stabilize :: Pipeline -> Dispatch IO RouteResponse
...
disp :: (Route, [(Text, EscapedText)]) -> Value -> Dispatch IO RouteResponse
...
lookupRoute :: Method -> Text -> Dispatch IO (Maybe (Route, [(Text, EscapedText)]))
